...
const deleteSitemapFromS3 = async () => {
  try {
    ...
    // Filter out the files that are within the "seo/" folder
    const filesToDelete = existingFiles.filter(file => file.startsWith(S3_PUBLIC_PATH));
...
const generateSitemapXml = async () => {
  ...
  for (const file of existingFiles) {
    await deleteSitemapFromS3(file);
  }
  ...
  for (const url of urls) {
    if (currentUrls.length >= MAX_URLS_PER_SITEMAP) {
      const sitemapFileName = `sitemap_part_${sitemapIndex}.xml`;
  ...
  await uploadToS3('sitemap_index.xml', generateSitemapIndex(sitemapFiles));
  console.log('Sitemap generated and uploaded successfully.');
};

const generateSitemapXmlContent = (urls) => {
  const urlset = create('urlset', { version: '1.0', encoding: 'UTF-8' }).att('xmlns', 'http://www.sitemaps.org/schemas/sitemap/0.9');
  urls.forEach(url => urlset.ele('url').ele('loc').txt(url));
  return urlset.end({ pretty: true });
};

const generateSitemapIndex = (sitemapFiles) => {
  const index = create('sitemapindex', { version: '1.0', encoding: 'UTF-8' }).att('xmlns', 'http://www.sitemaps.org/schemas/sitemap/0.9');
  sitemapFiles.forEach(file => index.ele('sitemap').ele('loc').txt(`https://s3.amazonaws.com/${S3_BUCKET}/${S3_PUBLIC_PATH}${file}`));
...
const uploadToS3 = async (fileName, fileContent) => {
  try {
    ...
    await s3Client.send(new PutObjectCommand({ Bucket: S3_BUCKET, Key: `${S3_PUBLIC_PATH}${fileName}`, Body: fileContent, ContentType: 'application/xml', ACL: 'public-read' }));
